...
dpid.128 = read.csv("C:/Users/Ssimmons/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/Projects/NanoBit-NRs/validation_chem.csv", stringsAsFactors = F, skipNul = T)
dpid.128 = as.data.table(dpid.128)

# read in control plate map#
control = read.csv("C:/Users/Ssimmons/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/Projects/NanoBit-NRs/agonist_control_chem.csv", stringsAsFactors = F, skipNul = T)
control = as.data.table(control)

# read in cohort dispense maps#
dispmap1 = read.csv("C:/Users/Ssimmons/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/Projects/NanoBit-NRs/AR/ARAgonist_Metabolism_Validation/valid_ar_agonist_cohort_1.map.csv", stringsAsFactors = F, skipNul = T)
dispmap1$cohort = 1
dispmap2 = read.csv("C:/Users/Ssimmons/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/Projects/NanoBit-NRs/AR/ARAgonist_Metabolism_Validation/valid_ar_agonist_cohort_2.map.csv", stringsAsFactors = F, skipNul = T)
dispmap2$cohort = 2
dispmap3 = read.csv("C:/Users/Ssimmons/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/Projects/NanoBit-NRs/AR/ARAgonist_Metabolism_Validation/valid_ar_agonist_cohort_3.map.csv", stringsAsFactors = F, skipNul = T)
dispmap3$cohort = 3
...
dispmap = data.table(Source.Plate = dispmap$Source.Plate.Barcode, Source.Well = dispmap$Source.Well, Destination.Well = dispmap$Destination.Well, 
                     ...
                     cpid = dispmap$cpid, cohort = dispmap$cohort)
...
# read in filemap #
filemap = read.csv("C:/Users/Ssimmons/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/Projects/NanoBit-NRs/AR/ARAgonist_Metabolism_Validation/AR_Agonist128_Met_filemap.csv", stringsAsFactors = F, skipNul = T)
...
for (j in 1:nrow(filemap)){
  ...
  file.name = paste("C:/Users/Ssimmons/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/Projects/NanoBit-NRs/AR/ARAgonist_Metabolism_Validation/data/TRno", filemap$TR[j], ".CSV", sep = "")
...
bmad.table = study[, mad(RLU), by = .(apid, chnm)][chnm == "DMSO",]
study$bmad = bmad.table$V1[match(study$apid, bmad.table$apid)]

pval1.table = study[, median(RLU), by = .(apid, chnm, logc)][chnm == "R1881" & logc > -1.1,]
study$pval1 = pval1.table$V1[match(study$apid, pval1.table$apid)]

pmad1.table = study[, mad(RLU), by = .(apid, chnm, logc)][chnm == "R1881" & logc > -1.1,]
study$pmad1 = pmad1.table$V1[match(study$apid, pmad1.table$apid)]

pval2.table = study[, median(RLU), by = .(apid, chnm, logc)][chnm == "DCLN" & logc > 1.9,]
study$pval2 = pval2.table$V1[match(study$apid, pval2.table$apid)]

pmad2.table = study[, mad(RLU), by = .(apid, chnm, logc)][chnm == "DCLN" & logc > 1.9,]
study$pmad2 = pmad2.table$V1[match(study$apid, pmad2.table$apid)]

study[, nval := ifelse(assay == "AR", log((RLU/bval), base = 2), (RLU - bval)/(pval2-bval)*100)]
setkey(study, chnm)
...
for (i in 1:length(unique(AR2$chnm))){
  ...
  sub.dt.min = min(min(sub.dt$resp), -2)
  sub.dt.max = max(max(sub.dt$resp), 2)
  ...
  curves_plot = curves_plot +
    ...
    coord_cartesian(ylim=c(sub.dt.min,sub.dt.max))+
    scale_colour_manual(values=cbbPalette, name = "Biogroup") +
    ylab("log-2 fold change over vehicle") +
    xlab("[cmpd] (log uM)") +
  ...
  file.path = "C:/Users/Ssimmons/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/Projects/NanoBit-NRs/AR/ARAgonist_Metabolism_Validation/plots/AR2"
...
AB2 = cmpd1[assay == "Viability",]
AB2$spid = AB2$biogroup
AB2$resp = AB2$nval

for (i in 1:length(unique(AB2$chnm))){
  for (m in 3:length(unique(AB2$biogroup))){
    
    chemtitle = NULL
    chemtitle = unique(AB2$chnm)[i]
    
    sub.dt = AB2[chnm == unique(AB2$chnm)[i] & biogroup %in% unique(AB2$biogroup)[c(1,2,m)],]
    #sub.dt = ar2.by.chnm[chnm == unique(ar2.by.chnm$chnm)[i] & biogroup %in% unique(biogroups[c(1,2,j)]),]
    ...
    file.name = paste(unique(AB2$chnm)[i], unique(AB2$biogroup)[m],"AB", "png", sep = ".")
    file.path = "C:/Users/Ssimmons/OneDrive - Environmental Protection Agency (EPA)/Profile/Desktop/Projects/NanoBit-NRs/AR/ARAgonist_Metabolism_Validation/plots/Alamar_Blue"
    ggsave(plot = curves_plot, filename = file.name, 
           path = file.path)
